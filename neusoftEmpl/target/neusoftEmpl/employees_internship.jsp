...
<head>
    ...
    <script src="./js/employees_internship.js"></script>
...
<div class="container main-container">
    ...
    <!-- 数据列表 -->
    <table id="employee_list" class="table table-bordered table-hover margin-top-20">
        <thead>
        <tr>
...
<script type="text/javascript">
  ...
  function build_emps_table(result){
    ...
    $.each(emps,function(index,item){
      ...
      var delBtn =  $("<button></button>").addClass("btn btn-warning btn-xs").attr("value",item.enId)
        .append($("<span></span>").addClass("glyphicon glyphicon-hand-left")).append("淘汰");

      var regordBtn = $("<button></button>").addClass("btn btn-success btn-xs").attr("value",item.enId)
      ...
      var displayBtn = $("<button></button>").addClass("btn btn-info btn-xs").attr("value",item.enId)
        .append($("<span></span>").addClass("glyphicon glyphicon-link")).append("录取");
  ...
  $(document).on("click",".btn.btn-warning.btn-xs",function(){
    ...
    var enId = $(this).val();
    //弹出确认对话框
    if(confirm("你确认淘汰 \" "+$(this).parents("tr").find("td:eq(1)").text()+"\" 吗?")){
      //发生ajax删除联系人
      $.ajax({
  ...
  //给录取按钮添加点击事件
  $(document).on("click",".btn.btn-info.btn-xs",function(){
    //首先需要获得要删除的联系人的姓名如何获得了,主要这里不能写成parent，暂时还没有弄明白
    //alert($(this).parents("tr").find("td:eq(1)").text());
    //删除联系人的id
    var enId = $(this).val();
    //弹出确认对话框
    if(confirm("你确认录取 \" "+$(this).parents("tr").find("td:eq(1)").text()+"\" 吗?")){
      //发生ajax删除联系人
      $.ajax({
        url:"/hzl/admissionEmployee",
        type:"post",
        data:"enId="+enId,
        dataType:"json",
        success:function(result){
