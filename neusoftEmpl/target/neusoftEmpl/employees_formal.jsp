...
<head>
    ...
    <script src="./js/employee.formal1.js"></script>
...
<div class="container main-container">
                    ...
                    <td class="fftd">
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td class="font3">
                                    职位：
                                    <select id="employeePost" name="job_id" style="width:143px;">
                                    ...
                                    身份证号码：<input type="text" id="employeeIdenity" name="cardId" maxlength="18">
                            ...
                            <tr>
                                <td class="font3">
                                    性别：
                                    <select name="sex" id="employeeSex" style="width:143px;">
                                        <option value="0">--请选择性别--</option>
                                        <option value="1">男</option>
                                        <option value="2">女</option>
                                    </select>
                                    手机：<input id="employeePhone" type="text" name="phone">
                                    所属部门：<select id="employeeDepartment"  name="dept_id" style="width:100px;">
                                    <option value="0">--部门选择--</option>
                                    <c:forEach items="${requestScope.depts }" var="dept">
                                        <option value="${dept.id }">${dept.name }</option>
                                    </c:forEach>
                                </select>&nbsp;
                                    <input type="button" onclick="selectEmployee(1)"  value="搜索"/>
                                    <input id="emp_delete_all_btn" type="button"  value="删除"/>
    ...
    <!-- 数据列表 -->
    <table id="employee_formal_list" class="table table-bordered table-hover margin-top-20">
        <thead>
        <tr id="emList">
...
<script type="text/javascript">
  ...
  function display() {
    $.ajax({
      ...
      success:function (result) {
        ...
        $("#employeePost").empty();
        var poList = result.polist;
        var str1='<option selected="selected" value="0">--请选择职位--</option>\n';
        var str2='';

        for(var i=0;i<poList.length;i++){
          str2+='<option  value="'+poList[i].poid+'">'+poList[i].poname+'</option>';
        }
        $("#employeePost").append(str1,str2);

        $("#employeeDepartment").empty();
        var deList = result.deList;
        var str3='<option selected="selected" value="0">--请选择部門--</option>\n';
        var str4='';

        for(var j=0;j<deList.length;j++){
          str4+='<option  value="'+deList[j].deId+'">'+deList[j].deName+'</option>';
        }
        $("#employeeDepartment").append(str3,str4);
      }
    })
  }
  ...
  function build_emps_table(result){
    //在构建之前先清空所有的数据
    $("#employee_formal_list tbody").empty();
    //第一步：获得所有的员工数据
    ...
    $.each(emps,function(index,item){
      ...
      var delBtn =  $("<button></button>").addClass("btn btn-warning btn-xs").attr("value",item.enId)
        .append($("<span></span>").addClass("glyphicon glyphicon-hand-left")).append("离职");

      var recordBtn= $("<button></button>").addClass("btn btn-success btn-xs").attr("value",item.enId)
        .append($("<span></span>").addClass("glyphicon glyphicon-align-left")).append("记录");

      var  trainBtn = $("<button></button>").addClass("btn btn-info btn-xs").attr("value",item.enId)
        .append($("<span></span>").addClass("glyphicon glyphicon-link")).append("培训");

      var btnTd = $("<td></td>").append(editBtn).append(" ").append(recordBtn).append(" ").append(trainBtn).append(" ").append(delBtn);
      ...
      $("<tr></tr>").append(allDeleteBtn)
        ...
        .append(enStatus)
        .append(btnTd)
        .appendTo("#employee_formal_list tbody");
    ...
    if(result.pageInfo.hasPreviousPage == false){
      ...
      prePageLi.click(function () {
        ...
        if(choose==1){
          //说明删除联系人成功
          //重新ajax请求到删除联系人的页面
          selectEmployee(result.pageInfo.pageInfo.pageNum - 1);
    ...
    if(result.pageInfo.hasNextPage == false){
      ...
      nextPageLi.click(function () {
        ...
        if(choose==1){
          //说明删除联系人成功
          //重新ajax请求到删除联系人的页面
          selectEmployee(result.pageInfo.pageNum + 1);
  ...
  function selectEmployee(e){
    ...
    var curPage=e;
    var enName = $("#employeeName").val();
    var enPost = $("#employeePost").find("option:selected").text();
    var enDepartment = $("#employeeDepartment").find("option:selected").text();
    var enSex = $("#employeeSex").find("option:selected").text();
    var enIdentity = $("#enIdentity").val();
    var enNumber=  $("#employeePhone").val();

    $.ajax({
      type:"post",
      url:"/hzl/selectFomalEmployee",
      data:{
        enName:enName,
        enPost:enPost,
        enDepartment:enDepartment,
        enSex:enSex,
        enIdentity:enIdentity,
        enNumber:enNumber,
        curPage:curPage,
      },
      dataType:"json",
  ...
  $(document).on("click",".btn.btn-primary.btn-xs",function() {
    ...
    var enId =$(this).val();
    window.open("/employee_update.jsp?enId="+encodeURI(enId));
